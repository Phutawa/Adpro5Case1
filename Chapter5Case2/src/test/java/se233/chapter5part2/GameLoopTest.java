...
public class GameLoopTest {
    ...
    @Test
    public void collided_snakeEatFood_shouldIncreaseScore() throws Exception {
        int initialScore = (int) ReflectionHelper.getField(gameLoop, "score");
        assertEquals(0, initialScore);
        clockTickHelper();
        int updatedScore = (int) ReflectionHelper.getField(gameLoop, "score");
        assertEquals(1, updatedScore);
    }
    @Test
    public void collided_snakeEatSpecialFood_shouldIncreaseScoreByFive() throws Exception {
        SpecialFood specialFood = new SpecialFood(new Point2D(0, 1));
        ReflectionHelper.setField(gameLoop, "food", specialFood);
        int initialScore = (int) ReflectionHelper.getField(gameLoop, "score");
        assertEquals(0, initialScore);
        clockTickHelper();
        int updatedScore = (int) ReflectionHelper.getField(gameLoop, "score");
    ...
    @Test
    public void keyProcess_shouldNotAllowOppositeDirection() throws Exception {
        ReflectionHelper.setField(snake, "direction", Direction.UP);
        ReflectionHelper.setField(gameStage, "key", KeyCode.DOWN);
        clockTickHelper();
        Direction currentDirection = (Direction) ReflectionHelper.getField(snake, "direction");
