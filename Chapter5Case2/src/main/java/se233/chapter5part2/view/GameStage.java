...
public class GameStage extends Pane {
    ...
    public void render(Snake snake, Food food) {
        ...
        if (food instanceof se233.chapter5part2.model.SpecialFood) {
    ...
    public void showGameOverPopup(int score) {
        if (Boolean.getBoolean("test.mode")) {
            System.out.println("Game Over popup triggered (test mode)");
            return;
        }
        Platform.runLater(() -> {
            ...
            alert.setContentText("Game Over! Your snake has died. Your score is " + score);
            alert.showAndWait();
        });
    }
